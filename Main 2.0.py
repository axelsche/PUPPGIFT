...
secondary_window_opened = False  # Initialize the flag
...
def setup_periodic_table_cells(frame, table, displayed):
    ...
    displayed.set(True)  # Set the displayed flag to True
...
def hide_periodic_table(frame, displayed):
    ...
    displayed.set(False)  # Set the displayed flag to False
...
def test_Row_Column_game(original_periodic_table_list, text_widget, entry_widget_row, entry_widget_col, attempts_label):
    global correct_answer, random_element
    ...
    if not hasattr(test_Row_Column_game, 'remaining_elements') or not test_Row_Column_game.remaining_elements:
        test_Row_Column_game.remaining_elements = original_periodic_table_list

    if not hasattr(test_Row_Column_game, 'current_game_elements') or not test_Row_Column_game.current_game_elements:
        test_Row_Column_game.current_game_elements = []

    random_element = random.choice(test_Row_Column_game.remaining_elements)
    test_Row_Column_game.remaining_elements.remove(random_element)

    correct_row = random_element['row']
    correct_column = random_element['col']
    correct_answer = (correct_row, correct_column)

    current_game_elements.append({
        'element': random_element['element'],
        'row': correct_row,
        'column': correct_column
    })

    text_widget.insert(tk.END, f"\nEnter the row and column for the randomly selected element ({current_game_elements[-1]['element']}): ")
    ...
    if user_row == correct_row and user_col == correct_column:
        ...
        if not test_Row_Column_game.remaining_elements:
            text_widget.insert(tk.END, "\nGame over. You have completed the game.")
            btn_fill_table.config(state=tk.DISABLED)  # Disable the Fill Table button
            test_Row_Column_game.remaining_elements = None  # Reinitialize the game state



        else:
            text_widget.insert(tk.END,
                               f"\nEnter the row and column for the next element ({test_Row_Column_game.remaining_elements[0]['element']}): ")
...
secondary_window_opened = False  # Variable to track if the secondary window has been opened

def open_secondary_window():
    global secondary_window, secondary_window_opened

    # Only open the secondary window if it hasn't been opened before
    if not secondary_window_opened:
        secondary_window = tk.Toplevel(root)
        secondary_window.title("Secondary Window")

        # Create a 9x18 grid of labels in the secondary window
        for i in range(10):
            for j in range(19):
                label = tk.Label(secondary_window, pady=5, padx=10, borderwidth=1, relief="solid")
                label.grid(row=i, column=j, padx=5, pady=5)

    # Clear the existing labels in the secondary window
    for widget in secondary_window.winfo_children():
        widget.destroy()
    ...
    # Populate the secondary window with updated elements
    for element in current_game_elements:
        row = element['row']
        col = element['column']
        label = tk.Label(secondary_window, text=element['element'])
        label.grid(row=row, column=col, padx=5, pady=5)

    # Set the variable to True after opening the window
    secondary_window_opened = True

def display_correct_answers(correct_elements):
    correct_answers_window = tk.Toplevel()
    correct_answers_window.title("Correct Answers")

    # Create a table to display correct answers
    table_frame = tk.Frame(correct_answers_window)
    table_frame.pack(padx=10, pady=10)

    headers = ["Element", "Abbreviation", "Row", "Column"]
    for col, header in enumerate(headers):
        tk.Label(table_frame, text=header, font=("Helvetica", 10, "bold"), padx=10, pady=5, borderwidth=1, relief="solid").grid(row=0, column=col)

    for row, element in enumerate(correct_elements, start=1):
        tk.Label(table_frame, text=element['element']).grid(row=row, column=0)
        tk.Label(table_frame, text=element['abbreviation']).grid(row=row, column=1)
        tk.Label(table_frame, text=element['row']).grid(row=row, column=2)
        tk.Label(table_frame, text=element['column']).grid(row=row, column=3)

    display_correct_answers(correct_elements)
...
def main():
    global root, btn_show_table, btn_test_proton, btn_test_abbreviation, btn_test_masses, btn_submit_answer, btn_hide_table,btn_fill_table, btn_exit, text_widget, entry_widget,entry_widget_row, entry_widget_col, attempts_label, original_periodic_table, table_frame

    root = tk.Tk()
    root.title("Periodic Table Game")

    displayed = tk.BooleanVar(value=False)  # Variable to track whether the periodic table is displayed
    ...
    btn_fill_table = tk.Button(button_frame, text="Fill Table", state=tk.NORMAL,
                               command=lambda: [test_Row_Column_game(original_periodic_table, text_widget,
                                                                     entry_widget_row, entry_widget_col,
                                                                     attempts_label), open_secondary_window()])
